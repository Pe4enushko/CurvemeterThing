...
public class CurvemeterBehaviour : MonoBehaviour
{
    ...
    void Start()
    {
        // Constant direction checking
        //InvokeRepeating("ChangeDirection", 0, 0.2f);
    ...
    void ChangeDirection(CurvemeterDirections direction)
    {
    ...
    private void EnterMap(Collider collision)
    {
        ...
        AddPoint(CurvemeterDirections.Forward);
            
        IsTouchingMap = true;
    }
    private CurvemeterDirections GetDirection()
    {
        Vector3 vel = (transform.rotation * thisRb.velocity);

        var velAbsX = Mathf.Abs(vel.x);
        var velAbsY = Mathf.Abs(vel.y);
        var velAbsZ = Mathf.Abs(vel.z);

        if (velAbsX > velAbsY && velAbsX > velAbsZ)
        {
            if (vel.x > 0)
            {
                return CurvemeterDirections.Forward;
            }
            else if (vel.x < 0)
            {
                return CurvemeterDirections.Backward;
            }
        }

        return CurvemeterDirections.Other;
    }
